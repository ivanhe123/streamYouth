...
texts = {
    "English": {
        "ERR_NO_RATE":"Please give the teacher a Feedback!", "RATED":"Finished Feedback this teacher at {time}! Thank You!","explanation":"Explain why did you give {name} this rating",
        "page_title": "PLE Youth Enrollment", "language_label": "Choose Language", "teacher_search_label": "Search for a teacher by name:","page_title_rate": "PLE Youth Rating",
        ...
        "logged_in": "Logged in as: {name}", "enrolled_label": "Enrolled Students", "no_enrollments": "No students enrolled yet.","no_rates":"You Didn't have any teacher to rate.",
        "not_enrolled": "Your name was not found in the enrollment.", "name_required": "Please enter your name to register.",
        "no_teachers_available": "No teachers are currently available for enrollment.", "enrollment_full": "Enrollment Full","no_teachers_rating_available": "No teacher available for you to rate.",
        ...
        "max_students_label": "Maximum Students (0=unlimited)", "class_status_header": "Class Status","class_rating": "Class Rating","class_no_rating": "No Ratings Yet", "enrollment_status_header": "Enrollment Status","rate_class":"Rate Your Experience with {name}",
    ...
    "中文": {
        ...
        "page_title": "PLE Youth 教师搜索与注册", "language_label": "选择语言", "teacher_search_label": "请输入教师姓名搜索：","page_title_rate": "PLE Youth 教师搜索与评分",
        ...
        "logged_in": "已登录: {name}", "enrolled_label": "已报名的学生", "no_enrollments": "当前没有报名的学生。","no_rates":"当前没有可反馈的老师",
        "not_enrolled": "未找到你的报名记录。", "name_required": "请输入你的名字以注册。",
        "no_teachers_available": "目前没有可报名的教师。", "enrollment_full": "报名已满","enrollment_closed": "此课程报名以关闭","no_teachers_rating_available":"你当前没有可评分的老师",
        ...
        "max_students_label": "最多学生数（0表示无限制）", "class_status_header": "课程状态","class_rating": "课程评分","class_no_rating": "还没有评分","enrollment_status_header": "报名状态","rate_class":"给你和{name}老师的学习经历打分",
        ...
        "manage_students_header": "管理已注册学生", "save_students_button": "保存对学生的更改","save_s-t_button":"保存更改",
...
def admin_route():
    ...
    assign_cols = ["Teacher", "Student", "Grade", admin_lang["raz_level_column"], admin_lang["location_column"],"_Student ID","_Teacher ID"]
    ...
    with add:
        ...
        if st.button(admin_lang["save_s-t_button"],key="s-t-ass-add"):
            def _():
                n_enrollments = {}
                for i,t in teacher_edit.iterrows():
                    tname=t["Teacher"]
                    if n_enrollments.get(tname) != None:
                        n_enrollments[tname].appened(t["_Student ID"])
                    else:
                        n_enrollments[tname] = [t["_Student ID"]]
                save_data(ENROLLMENTS_DB_PATH, n_enrollments)
            if (tevent.selection.rows==[]):
...
plain_id = params.get("eid", ""); plain_id = plain_id[0] if isinstance(plain_id, list) else plain_id
...
# --- STUDENT/USER PATH (Using secure_id for Enrollment) ---
    ...
    if not all_rate:
        ...
        # ... (Class Status display and buttons - unch
                ...
                if (ratt != None and ratt.isdigit() and int(ratt)>=0):
                    st_star_rating(label = lang["class_rating"], maxValue = 5, defaultValue = int(teacher_info.get("rating","0")), read_only = True )
                else:
        ...
        for n in active_teachers1:
            if enrollments.get(teachers_database.get(n).get("id"), False) and (secure_id in enrollments[teachers_database.get(n).get("id")]):
        ...
        # --- Display Teachers (Using secure_id for checks/actions) ---
        if not active_teachers: st.warning(lang["no_teachers_rating_available"])
        elif not filtered_teachers: st.error(lang["teacher_not_found_error"])
        else:
