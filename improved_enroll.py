...
def generate_teacher_id():
    """Generates a unique, obscure ID for a teacher."""
    # uuid4 generates a random UUID. .hex provides a 32-char hex string.
...
def admin_route():
    ...
    st.markdown("Add, edit, or remove teachers. 'Teacher ID' is auto-generated and read-only. 'Teacher Name' must be unique.")

    # Prepare data including the teacher ID
    teachers_list = []
    # Ensure every teacher has an ID (for backward compatibility if added before ID existed)
    needs_saving_for_ids = False
    temp_teachers_db = teachers_database.copy() # Operate on a copy for potential ID generation
    for name, details in temp_teachers_db.items():
        teacher_id = details.get("id")
        if not teacher_id:
            teacher_id = generate_teacher_id()
            details["id"] = teacher_id # Add ID to the details
            needs_saving_for_ids = True # Mark that we need to save back

        teachers_list.append({
            "Teacher ID": teacher_id, # Add ID here
            "Teacher Name": name,
    ...
    # If we generated any missing IDs, save the database back immediately
    if needs_saving_for_ids:
        save_data(TEACHERS_DB_PATH, temp_teachers_db)
        teachers_database = temp_teachers_db # Update the global variable
        st.info("Generated missing Teacher IDs. Data saved.")
        # Optionally rerun, but editing can continue
        # st.rerun()


    if not teachers_list:
        teachers_df = pd.DataFrame(columns=["Teacher ID", "Teacher Name", "Subject (English)", "Subject (Chinese)", "Grade"])
    else:
    ...
    edited_teachers_df = st.data_editor(
        ...
        key="teacher_editor",
        column_config={
             "Teacher ID": st.column_config.TextColumn("Teacher ID (Read Only)", disabled=True), # Display ID, make read-only
        ...
        column_order=("Teacher ID", "Teacher Name", "Subject (English)", "Subject (Chinese)", "Grade"),
        use_container_width=True
    )

    if st.button("Save Changes to Teachers"):
        # Keep a copy of the original state before edits to compare
        original_teachers_data = teachers_database.copy()
        new_teachers_database = {}
        error_occurred = False
        seen_names = set()
        processed_ids = set() # Track IDs present in the edited data

        for index, row in edited_teachers_df.iterrows():
            name = row["Teacher Name"]
            teacher_id = row["Teacher ID"] # Get the ID from the editor

            # --- Core Logic for Handling IDs ---
            # 1. If ID is missing or empty in the editor row (e.g., a newly added row)
            if pd.isna(teacher_id) or str(teacher_id).strip() == "":
                is_new_teacher = True
                teacher_id = generate_teacher_id() # Generate a new ID
            else:
                # 2. If ID exists, check if it corresponds to an existing teacher (by ID)
                # This handles name changes correctly
                is_new_teacher = True # Assume new unless we find a match by ID
                for _, details in original_teachers_data.items():
                    if details.get("id") == teacher_id:
                        is_new_teacher = False
                        break # Found existing teacher by ID
            ...
            if name in seen_names:
                 ...
                 error_occurred = True
                 continue
            # Check uniqueness against *other* existing teachers if the name changed for an existing ID
            if not is_new_teacher:
                 for existing_name, existing_details in original_teachers_data.items():
                     if existing_details.get("id") != teacher_id and existing_name == name:
                         st.error(f"Row {index+1}: Teacher Name '{name}' is already used by another teacher (ID: ...{existing_details.get('id', 'N/A')[-6:]}). Names must be unique.")
                         error_occurred = True
                         break # Break inner loop
                 if error_occurred: continue # Continue outer loop
        ...
        # --- Handle Deletions ---
        # Find teachers in the original data whose IDs are NOT in the processed data
        ...
        if not error_occurred:
            try:
                ...
                teachers_database = new_teachers_database # Update global immediately
                st.success("Teacher data updated successfully!")

                # Clean up enrollments for deleted teachers
                ...
                if enrollments_updated:
                     ...
                     enrollments = new_enrollments_after_teacher_delete # Update global enrollments
    ...
    # (Keep this section as is from the previous version)
    ...
    edited_students_df = st.data_editor(
        ...
        key="student_editor",
        column_config={
            "Encrypted ID": st.column_config.TextColumn("Encrypted ID (Read Only)", disabled=True),
    ...
    if st.button("Save Changes to Students"):
        ...
        # Get names *before* modifying user_database
        ...
        if not error_occurred:
            try:
                ...
                user_database = new_user_database # Update global
                ...
                for teacher, enrolled_students in current_enrollments.items():
                    cleaned_student_list = [ s_name for s_name in enrolled_students if s_name not in deleted_student_names ]
                    if cleaned_student_list:
                ...
                if enrollments_updated:
                    ...
                    enrollments = new_enrollments_after_delete # Update global
    ...
    assignments = []
    for teacher, students in enrollments.items(): # Use latest in-memory enrollments
    ...
    available_students = list(user_database.values())
    ...
    edited_assignments = st.data_editor(
        ...
        column_config={
            "Teacher": st.column_config.SelectboxColumn(
                "Teacher", options=available_teachers, required=True
            ),
            "Student": st.column_config.SelectboxColumn(
                "Student", options=available_students, required=True
            )
        },
        use_container_width=True
        )
...
if plain_id.lower() == "admin":
    ...
    filtered_teachers = { name: info for name, info in teachers_database.items() if not teacher_filter or teacher_filter.lower() in name.lower() }

    if not teachers_database: st.warning(lang["no_teachers_available"])
